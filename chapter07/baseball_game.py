...
def is_digit(user_input_number):
    ...
    try:
        ...
        if float(user_input_number) != user_number:
            result = False
        else:
            result = True
    except ValueError:
...
def is_duplicated_number(three_digit):
    ...
    number_list = [int(elem) for elem in three_digit]
...
def get_strikes_or_ball(user_input_number, random_number):
    ...
    for i in range(3):
        if user_input_number_list[i] == random_number_list[i]:
            strikes += 1
    for elem in user_input_number_list:
        if elem in random_number_list:
            intersection += 1
    ball = intersection - strikes
...
def baseball_game():
    ...
    while not is_no(answer) and not is_yes(answer):
        print("Wrong Input, Input again")
    if is_yes(answer):
        baseball_game()
